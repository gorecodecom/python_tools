...
def _build_filename(stem, extension, counter=None):
    ...
    return (
        f"{_truncate_utf8(stem, MAX_FILENAME_LENGTH - reserved_bytes)}{collision_suffix}{extension}"
    )
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "-k", "--keywords", default=str(DEFAULT_KEYWORDS_FILE), help="Path to keywords file"
    )
    parser.add_argument(
        "-r", "--recursive", action="store_true", help="Process folders recursively"
    )
    parser.add_argument(
        "-f", "--format", default="{date}_{title}", help="Filename format (default: {date}_{title})"
    )
    parser.add_argument(
        "-d", "--dry-run", action="store_true", help="Preview changes without renaming files"
    )
    ...
    while folder_path.lower() != "exit":
        ...
        if not pdf_files:
            ...
            for i, pdf_path in enumerate(pdf_files, 1):
                LOGGER.info("Processing file %s/%s: %s", i, len(pdf_files), pdf_path)
                success, _ = process_pdf(pdf_path, keywords, args.format, args.dry_run)
                if success:
                    success_count += 1
